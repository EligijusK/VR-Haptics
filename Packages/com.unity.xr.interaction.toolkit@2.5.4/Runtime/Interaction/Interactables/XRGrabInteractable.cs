...
namespace UnityEngine.XR.Interaction.Toolkit
{
    ...
    public partial class XRGrabInteractable : XRBaseInteractable
    {
        ...
        [SerializeField]
        float m_MovementAngleMaxVelocity = 10f;
...
#pragma warning restore 618
            ...
            m_Rigidbody.maxAngularVelocity = m_MovementAngleMaxVelocity;
            FlushRegistration();
        }
            ...
            switch (updatePhase)
            {
                ...
                case XRInteractionUpdateOrder.UpdatePhase.Fixed:
                    if (isSelected || isTransformDirty)
                    {
                        ...
                        if (m_CurrentMovementType == MovementType.PhysicsTracking && !m_SetupPhysicsTracking)
                        {
                            if (m_IsTargetLocalScaleDirty && !m_IsTargetPoseDirty && !isSelected)
                                ApplyTargetScale();
                    ...
                    if (interactorsSelecting.Count > 0 && m_SetupPhysicsTracking && !m_physicsTracking)
                    {
                        TrackWithPhysics(interactorsSelecting[0] as XRBaseInteractor, m_Rigidbody);
                        m_physicsTracking = true;
        ...
        private bool m_physicsTracking = false;
        private bool m_SetupPhysicsTracking = false;
        ...
        void PerformVelocityTrackingUpdateForTempPosition(XRInteractionUpdateOrder.UpdatePhase updatePhase, float deltaTime)
        {
            // Skip velocity calculations if Time.deltaTime is too low due to a frame-timing issue on Quest
            if (deltaTime < k_DeltaTimeThreshold)
                return;
            ...
            if (updatePhase == XRInteractionUpdateOrder.UpdatePhase.Fixed)
            {
                ...
                if (m_TrackPosition)
                {
                    ...
                    var positionDelta = m_AttachPointCompatibilityMode == AttachPointCompatibilityMode.Default
                        ...
                        : m_TargetPose.position - m_Rigidbody.worldCenterOfMass;
                    var velocity = positionDelta / deltaTime;
                    if (Vector3.Distance(m_TargetPose.position, transform.position) < 0.00005f && !m_SetupPhysicsTracking)
                    {
                        m_SetupPhysicsTracking = true;
                        ApplyTargetScale();
                        isTransformDirty = false;
                        return;
                    }
        ...
        protected override void OnSelectExiting(SelectExitEventArgs args)
        {
            ...
            if (interactorsSelecting.Count == 0)
            {
                ...
                DropPhysicsTracking(args.interactorObject as XRBaseInteractor);
                Drop();
        ...
        void TrackWithPhysics(XRBaseInteractor interactor, Rigidbody currentRigidbody)
        {
            if (m_CurrentMovementType == MovementType.PhysicsTracking)
            {
                FixedJoint joint = interactor.connectedRigidbody.gameObject.AddComponent<FixedJoint>();
                joint.connectedBody = currentRigidbody;
            }
        }
        ...
        void DropPhysicsTracking(XRBaseInteractor interactor)
        {
            if (m_CurrentMovementType == MovementType.PhysicsTracking)
            {
                interactor.connectedRigidbody.gameObject.TryGetComponent(out FixedJoint joint);
                m_physicsTracking = false;
                m_SetupPhysicsTracking = false;
                if (joint != null)
                    Destroy(joint);
            }
        }
        ...
        protected virtual void SetupRigidbodyGrab(Rigidbody rigidbody)
        {
            ...
            rigidbody.useGravity = m_CurrentMovementType == MovementType.PhysicsTracking;
#if UNITY_2023_3_OR_NEWER
