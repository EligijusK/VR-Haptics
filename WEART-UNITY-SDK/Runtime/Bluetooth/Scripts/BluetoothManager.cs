...
namespace WeArt.Bluetooth
{
    ...
    public class BluetoothManager
    {
        ...
        public async void StartScanning(Action<BleDevice> deviceFoundHandler, Action<BleDevice> deviceRemovedHandler)
        {
            ...
            if (statusCode > 0)
            {
                Debug.Log("Have an error code: " + statusCode);
                return;
            }
        ...
        private void UpdateScannedDeviceList(BleDevice device)
        {
            foreach (var scannedDevice in this.ListOfScannedDevices)
            {
                ...
                Debug.Log($"BL MANAGER SETS STATUS {device.DeviceMacAddress} BEFORE {GetDeviceStatus(device.DeviceMacAddress)}");
                this.ListOfScannedDevices[this.ListOfScannedDevices.IndexOf(scannedDevice)].DeviceStatus = DeviceStatus.Available;
                Debug.Log($"BL MANAGER SETS STATUS {device.DeviceMacAddress} AFTER {GetDeviceStatus(device.DeviceMacAddress)}");
                return;
            }
        ...
        {
            Debug.Log($"BL MANAGER SETS STATUS {deviceMacAddress} BEFORE {GetDeviceStatus(deviceMacAddress)}");
            ...
            Debug.Log($"BL MANAGER SETS STATUS {deviceMacAddress} AFTER {GetDeviceStatus(deviceMacAddress)}");
        }
        ...
        {
            Debug.Log($"{permissionName} Permission Denied And Dont Ask Again");
